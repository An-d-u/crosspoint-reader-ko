...
class ParsedText {
  ...
  bool isRubyAnnotatedWord(size_t wordIndex) const;
  void shiftRubyAnnotationsAfterInsert(size_t wordIndex);
  void extractLine(size_t breakIndex, int pageWidth, const std::vector<uint16_t>& wordWidths,
                   const std::vector<bool>& continuesVec, const std::vector<size_t>& lineBreakIndices,
                   const std::function<void(std::shared_ptr<TextBlock>)>& processLine, const GfxRenderer& renderer,
                   int fontId, int rubyFontId);
  std::vector<uint16_t> calculateWordWidths(const GfxRenderer& renderer, int fontId, int rubyFontId);
  void layoutCharacterWrap(const GfxRenderer& renderer, int fontId, int rubyFontId, uint16_t viewportWidth,
                           int spaceWidth,
 ...
 public:
  ...
  void layoutAndExtractLines(const GfxRenderer& renderer, int fontId, int rubyFontId, uint16_t viewportWidth,
