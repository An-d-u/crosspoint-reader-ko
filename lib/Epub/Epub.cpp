    ...
    if (Storage.openFileForRead("EBP", bmpPath, existingBmp)) {
      ...
      if (fileSize >= 74) {
        ...
        if (bmpCheck.parseHeaders() == BmpReaderError::Ok) {
          ...
          LOG_DBG("EBP", "Cache check: %dx%d %dbpp, rowBytes=%d, fileSize=%lu, expected=%lu", bmpCheck.getWidth(),
                  bmpCheck.getHeight(), bmpCheck.getBpp(), bmpCheck.getRowBytes(), fileSize, expectedSize);
